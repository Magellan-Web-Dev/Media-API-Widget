...
namespace MediaApiWidget\Updates;
 ...
 *  3. If the remote version is newer than MAW_PLUGIN_VERSION the plugin is injected
 ...
 *  (e.g. Media-API-Widget-3.4.0/).  The upgrader_post_install filter
...
final class GitHubUpdater
{
    ...
    private const REPO = 'Magellan-Web-Dev/Media-API-Widget';
    ...
    public static function addActionLinks(array $links): array
    {
        $check_url = wp_nonce_url(
            add_query_arg('action', 'maw_check_for_updates', self_admin_url('plugins.php')),
            'maw_check_for_updates',
            'maw_nonce'
        );
    ...
    public static function handleCheckForUpdates(): void
    {
        if (
            empty($_GET['action']) ||
            $_GET['action'] !== 'maw_check_for_updates' ||
            empty($_GET['maw_nonce']) ||
            !wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['maw_nonce'])), 'maw_check_for_updates') ||
        ...
        wp_safe_redirect(self_admin_url('plugins.php?maw_update_checked=1'));
        exit;
    }
    ...
    {
        if (empty($_GET['maw_update_checked']) || $_GET['maw_update_checked'] !== '1') {
            return;
        }
        ...
        if ($release && version_compare($release['version'], $current, '>')) {
            $msg = sprintf(
                'Media API Widget: Version <strong>%s</strong> is available. <a href="%s">Go to Updates</a>.',
                esc_html($release['version']),
                esc_url(self_admin_url('update-core.php'))
            );
        } elseif ($release) {
            $msg = 'Media API Widget: You are running the latest version.';
        } else {
    ...
    private static function getLatestRelease(): ?array
    {
        ...
        $response = wp_remote_get(self::API_URL, [
            ...
            'headers' => [
                ...
                'User-Agent' => 'WordPress/Media-API-Widget',
            ],
        ]);
     ...
     *
     * Example: media-api-widget/media-api-widget.php
     *
     * @return string
     */
